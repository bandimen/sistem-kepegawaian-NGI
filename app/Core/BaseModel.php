...
class BaseModel extends Model
{
	...
	function log_api_fare($data)
	{
		$builder = $this->db->table('s_log_api_fare');
		$builder->insert($data);
	}
	...
	function base_insertId($data, $table)
	{
		$builder = $this->db->table($table);
	...
	function base_load_datatable_orderby($baseQuery, $whereQuery, $whereTerm, $start, $length, $orderColumn, $orderDirection, $groupBy = NULL, $orderBy = NULL)
	{
		$q = ($whereTerm != "" ? $baseQuery . " and (" . implode(" or ", array_map(function ($x) use ($whereTerm) {
			return $x == "json" ? "JSON_SEARCH(" . $x . ", 'one', ?, '', '$[*]')" : $x . " like ?";
		}, $whereQuery)) . ")" : $baseQuery) . ($groupBy != NULL ? "GROUP BY " . implode(", ", $groupBy) : "") . " ORDER BY " . $orderBy;
	...
	function base_load_select2hubdat($baseQuery, $whereField, $keyword, $page, $perpage, $orderBy = NULL)
	{
		$q = $whereField != "" ? $baseQuery . " and (" . implode(" or ", array_map(function ($x) use ($keyword) {
			return $x . " like ?";
		}, $whereField)) . ")" : $baseQuery;
		...
		if ($orderBy == NULL) {
			return $whereField != "" ? $this->db2->query($q, $whereKey)->getResult() : $this->db->query($q)->getResult();
		} else {
			return $whereField != "" ? $this->db2->query($q, $whereKey)->getResult() : $this->db->query($q)->getResult();
		}
	}
